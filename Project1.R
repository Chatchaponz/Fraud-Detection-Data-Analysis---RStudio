...
claims_df %>% select(Consumer_profile, Fraud) %>%
  ...
  coord_polar(theta = "y") +
  scale_fill_manual(values = c("#38b6ff","#004bad")) +
  theme_void() + theme( strip.text.x = element_text( size = 25, face = "bold") )
...
claims_df %>% select(Consumer_profile , City , Fraud) %>% 
  ...
  facet_wrap(vars(Consumer_profile)) + labs(x = "Count") +
  theme_bw() + theme(strip.background = element_rect(fill = "white"),
...
claims_df %>% select(City , Fraud) %>% 
  ...
  coord_polar(theta = "x") +
  theme_void() + theme(strip.text.x = element_text( face = "italic")) +
  scale_fill_manual(values = c("#38b6ff","#004bad"))
...
claims_df %>% select(Purpose , Fraud ) %>% 
  ...
  geom_bar(position = "fill") + labs(y = "Density") +
  theme_bw() + theme(strip.background = element_rect(fill = "white")) +
  scale_fill_manual(values = c("#38b6ff","#004bad"))
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
  claims_df %>% select(Purpose , Fraud ,City) %>% 
    ggplot(aes(x = Purpose , fill = Fraud)) +
    geom_bar(position = "fill") +
    facet_wrap(vars(City)) + labs( y = "Density" ) +
    theme_bw() + theme(strip.background = element_rect(fill = "white"),
                       strip.text.x = element_text( face = "bold"),
...
#Area and Fraud
  ...
  claims_df %>% select(Area, Fraud ) %>%
    ...
    geom_bar(position = "fill") + labs( y = "Density" ) + theme_bw() +
    scale_fill_manual(values = c("#38b6ff","#004bad"))
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
  claims_df %>% select(State, Fraud ) %>%
    ...
    geom_bar(position = "fill") + labs( x = "Count" ) + theme_bw() +
    scale_fill_manual(values = c("#38b6ff","#004bad"))
...
#Purchased_from Claim_Value , Fraud
  ...
  claims_df %>% select(Purchased_from, Claim_Value ,Fraud ) %>%
    ...
    facet_wrap(vars(Purchased_from)) + labs(x = "Claim Value", y = "Density") +
    theme_bw() + theme(strip.background = element_rect(fill = "white"),
                       strip.text.x = element_text( face = "bold")) +
